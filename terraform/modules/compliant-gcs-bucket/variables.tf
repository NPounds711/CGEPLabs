...
variable "location" {
  type        = string
  description = "GCS bucket location. Multi-regions (US, EU) are valid for buckets."
  default     = "us-central1"
}
...
variable "project_label" {
  type        = string
  description = "Short project identifier. Becomes part of bucket names and the project label."
...
variable "environment" {
  type        = string
  description = "Deployment environment. Drives the environment label and retention enforcement."
...
variable "retention_days" {
  type        = number
  description = "Object retention in days. Production requires >= 365 (AU-11)."
